...
- 同梱の最小リファレンス戦略: EMA クロスオーバーによるスキャルピング戦略（[`pkg/strategy/scalping/README.md`](./pkg/strategy/scalping/README.md)。RSI / クールダウン付きの拡張リファレンス実装は [`example/strategy/scalping/`](./example/strategy/scalping/)）
- エンジンレベルのリスク管理（日次損失上限・トレード別損失上限・最小取引間隔・1日の取引回数上限 — `trading.risk_management` で設定）
- WebUIによる取引の開始・停止制御
...
- **データベース**: SQLite（軽量・埋め込み・外部DB不要）
  - DB保持: `retention_days` で設定可能（example のデフォルト: 90日。未設定時のコード既定値は 1 日）
  - 過去データ: bitFlyerで確認可能
...
| ドキュメント | 内容 |
|---|---|
| [docs/CONFIG.ja.md](docs/CONFIG.ja.md) | 設定リファレンス |
| [docs/STRATEGY.ja.md](docs/STRATEGY.ja.md) | 取引戦略リファレンス（プラガブルアーキテクチャ概要・同梱戦略一覧） |
| [docs/DESIGN_DOC.ja.md](docs/DESIGN_DOC.ja.md) | アーキテクチャ設計ドキュメント（**カスタム戦略の実装方法** § 5） |
| [docs/DATA_MANAGEMENT.ja.md](docs/DATA_MANAGEMENT.ja.md) | データ管理リファレンス |
