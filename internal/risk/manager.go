...
func NewRiskManager(
	riskCfg *config.RiskManagementConfig,
	tradingCfg *config.TradingConfig,
	tradingRepo TradingRepository,
	analyticsRepo AnalyticsRepository,
	trader trading.Trader,
...
func (rm *Manager) checkTradeAmount(signal *strategy.Signal, totalBalanceJPY float64) error {
	...
	if signal.Action == strategy.SignalBuy {
		feeRate := rm.tradingCfg.FeeRate
		tradeAmount *= (1 + feeRate)
	}

	maxAmount := totalBalanceJPY * rm.config.MaxTradeAmountPercent / 100

	if tradeAmount > maxAmount {
		return fmt.Errorf("trade amount %f (including fees) exceeds maximum %f (%.1f%% of balance %f)",
			tradeAmount, maxAmount, rm.config.MaxTradeAmountPercent, totalBalanceJPY)
	}
...
func (rm *Manager) checkDailyTradeLimit() error {
	...
	maxDailyTrades := rm.config.MaxDailyTrades
...
func (rm *Manager) checkTradeInterval() error {
	...
	minInterval := rm.config.MinTradeInterval

	// Parse time interval from configuration (e.g., "5m" -> 5 minutes)
	duration, err := time.ParseDuration(minInterval)
	if err != nil || minInterval == "" {
		if rm.logger != nil {
			rm.logger.System().WithError(err).Warn("Invalid min_trade_interval format, using default 5m")
		}
...
func (rm *Manager) checkTotalLossLimit(ctx context.Context) error {
	...
	totalAssets := rm.tradingCfg.InitialBalance
	for _, bal := range balances {
		if bal.Currency == "JPY" {
	...
	// Calculate loss percentage against current assets (not initial balance)
	lossPercent := (totalLoss / (totalAssets + totalLoss)) * 100
	maxLossPercent := rm.config.MaxTotalLossPercent
