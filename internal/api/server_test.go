...
func TestHandleOrders(t *testing.T) {
	...
	// Save trading data (treated as order) for testing
	trade := &domain.Trade{
...
func TestHandlePerformance(t *testing.T) {
	...
	var metrics []domain.PerformanceMetric
...
func TestHandleConfig_Post(t *testing.T) {
	...
	// Config update request for testing (mode is removed as it's fixed at runtime)
	...
	// Note: configPath field does not exist, so this configuration is omitted
...
func TestSetApplication(t *testing.T) {
	...
	// Verify that application is set (verified indirectly due to internal state)
	req := httptest.NewRequest(http.MethodPost, "/api/strategy/reset", nil)
	w := httptest.NewRecorder()
