...
func (om *OrderMonitor) MonitorExecution(ctx context.Context, result *domain.OrderResult) {
	...
	timeout := time.After(om.monitorTimeout)

	for {
		select {
		case <-ctx.Done():
			if om.logger != nil {
				om.logger.Trading().WithField("order_id", result.OrderID).Info("Order monitoring canceled by context")
			}
			// Try to get final order status before returning
			if err := om.saveFinalOrderState(ctx, result); err != nil && om.logger != nil {
		...
		case <-timeout:
			if om.logger != nil {
				om.logger.Trading().WithField("order_id", result.OrderID).Warn("Order monitoring timeout - checking final status")
			}
			...
			return
		case <-ticker.C:
			if om.checkOrderStatus(ctx, result) {
				return
			}
		}
	}
...
func (om *OrderMonitor) saveFinalOrderState(ctx context.Context, result *domain.OrderResult) error {
	// Use a fresh short-lived context: the parent ctx may already be expired/cancelled
