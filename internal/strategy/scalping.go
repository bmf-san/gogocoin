...
// ScalpingConfig is the configuration for minimal scalping strategy
type ScalpingConfig struct {
	EMAFastPeriod  int     `yaml:"ema_fast_period"`  // Fast EMA period (e.g., 9)
	EMASlowPeriod  int     `yaml:"ema_slow_period"`  // Slow EMA period (e.g., 21)
	TakeProfitPct  float64 `yaml:"take_profit_pct"`  // Take profit % (e.g., 0.8)
	StopLossPct    float64 `yaml:"stop_loss_pct"`    // Stop loss % (e.g., 0.4)
	CooldownSec    int     `yaml:"cooldown_sec"`     // Cooldown seconds (e.g., 90)
	MaxDailyTrades int     `yaml:"max_daily_trades"` // Max trades per day (e.g., 3)
	MinNotional    float64 `yaml:"min_notional"`     // Minimum order amount (e.g., 200)
	FeeRate        float64 `yaml:"fee_rate"`         // Fee rate (e.g., 0.001)
}

// NewScalping creates a new minimal stateless scalping strategy
func NewScalping(config ScalpingConfig) *ScalpingStrategy {
...
func (s *ScalpingStrategy) calculateQuantity(price float64) float64 {
	if price <= 0 {
		return 0.0
	}

	// Calculate quantity to meet minimum notional (including fees)
	// notional = price * quantity
	// With fees: total_cost = notional * (1 + fee_rate)
