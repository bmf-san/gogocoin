...
func TestScalpingStrategy_GenerateStatelessSignal(t *testing.T) {
	...
	strategy := NewScalping(ScalpingConfig{
		EMAFastPeriod:  9,
		EMASlowPeriod:  21,
		TakeProfitPct:  0.8,
		StopLossPct:    0.4,
...
func TestScalpingStrategy_CalculateQuantity(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: 0.00002,    // 200 / 10000000 = 0.00002 BTC
		},
		{
			name:     "eth_price",
			price:    400000.0, // 400K JPY per ETH
			expected: 0.0005,   // 200 / 400000 = 0.0005 ETH
		},
		{
			name:     "xrp_price",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			qty := strategy.calculateQuantity(tt.price)
