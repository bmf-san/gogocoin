...
func NewMaintenanceWorker(logger *logger.Logger, db domain.MaintenanceRepository, retentionDays int) *MaintenanceWorker {
...
func (w *MaintenanceWorker) Run(ctx context.Context) {
	...
	// Track last execution date to prevent duplicates
	now := time.Now()
	lastCleanupDate := now.Format("20060102") // YYYYMMDD, initialize to today

	// Run initial cleanup on startup if we missed today's midnight cleanup
