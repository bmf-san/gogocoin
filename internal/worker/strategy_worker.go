...
func (w *StrategyWorker) Run(ctx context.Context) {
	...
	for {
		...
		case marketData, ok := <-w.marketDataCh:
			...
			go func(data domain.MarketData) {
				...
				w.logger.Strategy().WithField("symbol", data.Symbol).WithField("price", data.Price).Info("Market data received in strategy worker")
				...
				w.logger.Strategy().WithField("symbol", data.Symbol).WithField("history_count", len(historyCopy)).Info("Market data added to history")

				// Execute strategy on market data receipt (pass only history for the relevant symbol)
				w.logger.Strategy().WithField("symbol", data.Symbol).Info("Executing strategy on market data update")
