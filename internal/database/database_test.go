...
func TestSaveMarketData(t *testing.T) {
	...
	// 保存されたmarket dataof確認is複雑なofwith、errorが発生しないことだけ確認
}
...
func TestSavePerformanceMetric(t *testing.T) {
	...
	// 保存されたperformancemetricsgetand verify
...
func TestGetLogs_WithLevelFilter(t *testing.T) {
	...
	// 全logget（レベルフィルタリングなし）
	logs, err := db.GetLogs(10)
	if err != nil {
		t.Errorf("Failed to get logs: %v", err)
	}

	// 4つofレベルすべてが保存されているか確認
	if len(logs) != 4 {
		t.Errorf("Expected 4 logs, got %d", len(logs))
	}

	// 最新oflog（最後に保存されたDEBUG）" "確認
...
func TestConcurrentAccess(t *testing.T) {
	...
	// 全てof goroutine of完了" "待つ
	for i := 0; i < 10; i++ {
		<-done
	}

	// 全てoftradeが保存されているか確認
